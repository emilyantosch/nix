{ config, ... }:
{
  programs.fish = {
    ...
    shellAliases = {
      ...
      ## pi bindings
      po = "aws-sso exec -p ai-coding.tools -- pi --provider amazon-bedrock --model eu.anthropic.claude-opus-4-6-v1";

      # jira bindings
      jl = ''jira issue list --component "Fleet Optimization" --status "~Closed"'';
      jc = ''jira issue create --component "Fleet Optimization" --status "Open"'';

      # kubernetes bindings
      ku = "kubectl -n fleet-optimization";
    ...
    interactiveShellInit = ''
                      ...
                      export ARTIFACTORY_APIKEY=$(cat /run/secrets/artifactory)
                      export JIRA_API_TOKEN=$(cat /run/secrets/jira)
      ...
      function eksenv
      	export KUBECONFIG="$HOME/.kube/moia-01-$argv"
      	aws-sso exec -p "k8s.$argv.eu-central-1" -- fish
      end
